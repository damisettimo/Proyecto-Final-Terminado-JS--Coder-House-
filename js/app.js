...
//Lo primero que hace es mostrar el carrito antes de cargar la pagina con DOMContentLoaded
...
botonVaciar.addEventListener('click', () => {
    ...
    Swal.fire({
      ...
      })// Alerta sweetAlert que pregunta si esta seguro de vaciar el carrito
...
//Agregando etiquetas a HTML con Fetch a traves de Json simulando una API y renderiza dinamicamente
fetch("./stock.json")
...
.then(function (stockJson) {
        stockJson.forEach((prod) => {
            ...
            stockProductos.push(prod) //Llena Stock en StockProductos 

            console.log (stockProductos)
            contenedorProductos.appendChild(div);

            //Boton de agregar carrrito
            let boton = document.getElementById(`agregar${prod.id}`); 

            //Funcion que agrega productos al Carrito y muestra un Sweetalert
...
//Funcion que Elimina Producto de carrito
const eliminarDelCarrito = (prodId) => {
    ...
    //MODIFICA EL CARRITO
    Swal.fire('El Producto seleccionado fue eliminado del carrito.') //Etiqueta Sweetalert confirma eliminacion
    console.log(carrito)
}
...
const actualizarCarrito = () => {
    ...
    carrito.forEach((prod) => {
        ...
        //Guarda productos en localStorage y convierte en Json
